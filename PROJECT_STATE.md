...
## v57 : Ping-pong — victoire à 7 points + écran de fin refait (croisement d'un rapport Gemini avec l'analyse vidéo)
L'utilisateur a fourni un rapport technique généré par Gemini listant des mécaniques à répliquer. Confrontation systématique avec les 2 vidéos déjà analysées image par image avant d'appliquer quoi que ce soit :
- ✅ **Adopté, double-confirmé** (vidéo v56 + rapport Gemini, indépendamment) : victoire au premier à **7 points** (`PP_WIN_SCORE` 11→7 — partie plus courte, cohérent avec l'objectif "addictif" du début du projet). Écran de fin refait en **overlay** par-dessus le plateau figé (au lieu de le masquer) : voile sombre, pluie de 22 étoiles jaunes en CSS (`@keyframes ppStarFall`), médaille or/argent avec ruban (`#ping-victory-medal`), 3 boutons (🏠 Accueil → `openJeux()`, REJOUER en grand → `ppRestartSameMode()`, 📊 Stats → affiche/masque le classement fictif existant via `ppToggleVictoryStats()`).
- ❌ **Rejeté, contredit par les 2 vidéos** : le rapport Gemini décrit un rebond élastique standard sur les murs latéraux ET une balle qui resterait "jouable en dehors de la table" après être sortie. Aucune des deux vidéos analysées image par image (v55 et v56) ne montre ça — les deux montrent une trajectoire diagonale continue, sans rebond, jusqu'à sortie franche du terrain. Gemini semble avoir décrit un pong générique par défaut plutôt que le jeu réellement observé. **Non appliqué** — le comportement sans rebond (v55) est conservé. À rouvrir si l'utilisateur a une vidéo montrant explicitement ce cas.
- ❌ **Rejeté, contredit par la mesure pixel de la photo de référence initiale** : Gemini suggère une table à 70-75% largeur / 60-65% hauteur de l'écran. La toute première photo de référence fournie par l'utilisateur, mesurée précisément au pixel (voir v52-v54), donnait ~97% largeur / ~72% hauteur — et l'utilisateur avait explicitement demandé une table aussi grande que sur cette photo. **Non appliqué** — la taille actuelle (90%/93%, ajustée en v55 suite au retour "un peu moins gros") est conservée.
- 🟨 **Non confirmé par vidéo, non implémenté** : le service "actif" (balle statique devant la raquette jusqu'au premier coup du joueur) décrit par Gemini n'a pas été vu explicitement sur les vidéos analysées (aucune ne montre le tout début d'un point). Service auto-alterné actuel conservé faute de preuve contraire.
...
## v59 : Ping-pong — v58 était un contresens complet, refonte selon les vraies règles du ping-pong
L'utilisateur a explicitement rejeté v58 : "tu m'as fait un air hockey alors que je te demande un putain de PING PONG". Correction de fond, cette fois-ci actée clairement :
- **Mur latéral supprimé (encore, définitivement)** : au vrai ping-pong il n'y a pas de mur. `ppStepBall` ne contraint plus x du tout — la balle continue tout droit même hors-table.
- **Sortie latérale ≠ point** : un point ne se marque QUE quand la balle dépasse le fond (derrière l'un des 2 joueurs), jamais sur le côté. `ppCheckScore` ne vérifie que y (déjà le cas depuis v57, confirmé correct).
- **La balle et les raquettes peuvent sortir de la table** : jusqu'ici le `<canvas>` faisait exactement la taille de la table (0..ppW/0..ppH) donc tout ce qui sortait des bords était juste invisible (coupé par le canvas). Restructuration : le `<canvas>` est maintenant PLUS GRAND que la table dessinée dedans (`ppCanvasW/ppCanvasH`), avec une marge hors-table de chaque côté (`ppOffX`/`ppOffY`, ~12%/10% de l'espace dispo). La table (rectangle vert) est dessinée décalée de cette marge (`ctx.translate(ppOffX, ppOffY)` en début de `ppRender`), et les coordonnées tactiles (`ppLocalXY`) retirent ce même décalage pour rester en "espace table" cohérent avec toute la physique existante. Les raquettes peuvent désormais suivre le doigt/la balle bien au-delà du bord vert (`ppClampPaddle` autorise ~85% de la marge), et l'IA du bot ne borne plus sa cible à l'intérieur de la table (elle poursuit la balle où qu'elle soit, seule la position finale est bornée par la marge).
- **Vrai rebond AU SOL ajouté** (celui qui manquait vraiment, différent du mur) : simulation d'une hauteur `z` avec gravité (`PP_GRAVITY`), relancée à chaque frappe (`vz0` = `PP_BOUNCE_VZ0`) et à chaque service. La balle "monte" visuellement puis retombe, touche la table une fois (`ppBall.bounced`), avec une gerbe de particules à l'impact (`ppSpawnBounceMark`, ex-`ppSpawnWallSpark` renommée). Rendu : le sprite de la balle se soulève avec `z` (`ppBall.y - z*0.6`) pendant que son ombre reste au sol à la position réelle — les deux se rejoignent exactement au moment du rebond. Le rebond est purement visuel/temporel (ne conditionne pas la détection de collision raquette, pour rester accessible au toucher).
- **Table conservée à 73%/65%** (v58, non remis en cause par ce message).
- **Testé** : simulation headless réécrite (plus de mur, cible IA non bornée, raquettes avec débordement ~10% de la largeur) — 50 matchs, 0 anomalie.
- **Limite assumée** : le rebond au sol est réglé avec des constantes ajustées à l'estimation (pas de vrai moteur physique), et l'ensemble reste non testable en rendu réel depuis ce sandbox — le ressenti exact (timing du rebond, générosité du débordement hors-table) demande un retour après test.

## v60 : Ping-pong — vraies règles du ping-pong (cahier des charges détaillé fourni par l'utilisateur)
L'utilisateur a fourni un cahier des charges précis (terminologie, machine à états, matrice de décision) pour les règles officielles. Implémentation d'une vraie machine à états de volée, remplaçant le scoring simpliste "ça sort derrière = point" par le cycle réglementaire complet :
- **`ppLegalToHit`** (bool) : la balle ne peut être touchée qu'après avoir fait tous ses rebonds réglementaires pour le trajet en cours. Y toucher avant = **faute de volée**, point pour `ppLastHitter` (celui qui vient de la frapper).
- **`ppPendingCheckpoints`** : file ordonnée de positions Y à franchir avant que le relanceur ait le droit de toucher la balle.
  - **Retour normal** : 2 checkpoints — franchissement du filet (`net:true`, risque de faute de filet), puis rebond côté relanceur juste après le filet (`net:false`).
  - **Service** : 3 checkpoints — rebond côté serveur d'abord (nouveau : la balle part maintenant du camp du serveur, pas du centre), franchissement du filet, rebond côté relanceur.
- **Faute de filet** : à chaque franchissement du filet, tirage probabiliste basé sur la puissance du coup (`ppSetupReturnCheckpoints`) — frappe molle (<22% de puissance) : 14% de risque ; frappe moyenne (<35%) : 5% ; frappe normale/forte : jamais. Le service ne finit jamais dans le filet (simplification assumée). Point pour l'adversaire de celui qui a frappé.
- **"Out" avant rebond** vs **échec de retour après rebond** — même symptôme visuel (la balle sort derrière la ligne de fond) mais arbitrage différent selon `ppLegalToHit` au moment de la sortie (`ppCheckScore` revu) : si la balle n'a pas encore fait son rebond réglementaire, c'est une sortie "dehors" — faute de l'attaquant, point au défenseur ; si elle avait déjà rebondi validement, c'est un échec de retour — point à l'attaquant (`ppLastHitter`), comportement déjà en place avant.
- **Double rebond** : non simulé littéralement comme un 2e rebond physique distinct (le modèle ne fait pas repartir la balle après un 1er rebond raté) — son issue de jeu (le défenseur ne renvoie pas après un rebond valide) est déjà couverte par le cas "échec de retour après rebond" ci-dessus. Simplification assumée et documentée.
- **Rebond visuel (z)** resynchronisé sur les checkpoints réglementaires (au lieu d'un timing de gravité indépendant comme en v59) — la balle se fige au sol entre deux checkpoints puis relance son arc pile au bon moment logique, pour que le rebond visible corresponde à l'événement de règle, pas juste à une esthétique décorative.
- **Testé** : nouvelle simulation headless dédiée (`pp_rules_sim.js`, hors repo) reproduisant exactement les mêmes fonctions que le jeu réel (checkpoints, faute de volée, faute de filet, out) — 80 matchs bot-vs-bot, 0 anomalie, 0 blocage. Test ciblé confirmant que le tirage probabiliste de faute de filet se déclenche bien. Note : en conditions bot-vs-bot normales, les fautes de volée/filet/out ne se sont pas déclenchées sur cet échantillon (l'IA reste rarement assez agressive près du filet et frappe rarement assez mollement) — le mécanisme est validé fonctionnel mais restera sans doute un événement plutôt rare en jeu, ce qui est cohérent avec le vrai ping-pong.
- **Limite assumée, comme toutes les versions précédentes** : non testable en rendu/ressenti réel depuis ce sandbox — en particulier le fait qu'une frappe trop tôt (avant le rebond) soit sanctionnée peut demander un ajustement du ressenti après un vrai test (retour utilisateur nécessaire pour savoir si c'est trop punitif ou juste).

## v61 : Ping-pong — v60 attribuait parfois le point au mauvais joueur, remplacé par une règle simple et fiable
Retour utilisateur après test réel : "je gagne un point quand je tape la balle et qu'elle sort de mon côté" — bug confirmé. La machine à états à 3 checkpoints de v60 (rebond serveur → filet → rebond relanceur, avec faute de volée et faute de filet) était trop complexe et produisait des résultats incorrects dans certains cas non reproduits en simulation bot-vs-bot (l'IA des bots ne déclenchait jamais les fautes de volée/filet dans mes tests, donc ce chemin de code n'a jamais été vraiment exercé avant le push — leçon retenue).
**Remplacé par une règle unique, simple et vérifiable** : `ppTargetBounced` (bool) — la balle doit obligatoirement toucher la table du **camp adverse** du dernier frappeur avant de sortir.
- Rebond sur son PROPRE côté (une balle molle qui retombe de son côté à soi) → ne valide RIEN, `ppTargetBounced` reste `false`.
- Si la balle sort (dépasse une ligne de fond) alors que `ppTargetBounced` est `true` (elle a bien touché le camp adverse) → point à celui qui l'a frappée (l'adversaire a raté son retour).
- Si elle sort alors que `ppTargetBounced` est `false` (jamais touché le camp adverse) → **celui qui l'a frappée en dernier perd le point**, exactement la règle donnée par l'utilisateur mot pour mot : *"le dernier qui touche la balle et qui fait sortir la balle perd le point"*.
- Suppression complète de la faute de volée et de la faute de filet (v60) — supprimées avec le système de checkpoints qui les portait, pour réduire la surface de bug. Peuvent être réintroduites plus tard sur une base plus simple si demandé.
- Rebond visuel (z) simplifié en retour à la version v59 (bounce-once-then-flatten, indépendant), plus besoin de le resynchroniser sur des checkpoints qui n'existent plus.
- **Testé** : nouvelle simulation dédiée (`pp_simple_rules_sim.js`) — 80 matchs bot-vs-bot, 0 anomalie. Cette fois les DEUX types d'issue se sont bien produits et ont été comptés séparément (740 échecs de retour après rebond valide, 141 sorties avant d'avoir touché le camp adverse — donc point perdu par le frappeur), preuve que le chemin de code corrigé est réellement exercé, contrairement à v60.
- **Limite assumée** : toujours non testable en rendu réel depuis ce sandbox — retour nécessaire pour confirmer que l'attribution des points est maintenant correcte dans tous les cas.

## v62 : Ping-pong — la vraie cause : le rebond ne vérifiait pas d'être DANS la table
Retour utilisateur : "le bot tape la balle et la sort, dans ce cas c'est moi qui suis censé marquer le point" — toujours pas correct après v61. Cause identifiée : `ppStepBall` ne validait un rebond "camp adverse" (`ppTargetBounced = true`) qu'en vérifiant la **moitié Y** (haut/bas), jamais si la balle était réellement **dans la largeur de la table** (X entre 0 et `ppW`) à ce moment. Or depuis v59, la balle peut voler hors-table sur le côté (chassée par les raquettes) — un tir hors-table qui "touche le sol" (z=0) dans la bonne moitié Y mais largement à côté de la table (x hors table) comptait quand même comme un point valide gagné, alors qu'il n'a jamais touché la table.
**Fix** : ajout de la vérification `b.x >= 0 && b.x <= ppW` en plus de la zone Y dans `ppStepBall` — un rebond ne valide `ppTargetBounced` que s'il a lieu à la fois du bon côté ET dans les limites réelles de la table. Exactement la demande de l'utilisateur : "il faut bien délimiter la table comme la zone où la balle est censée la toucher".
- **Testé** : test ciblé confirmant qu'un rebond à x=-15 (hors table) ne compte plus comme touche valide (avant : `true`/bug, après : `false`/correct) + simulation complète (80 matchs, 0 anomalie, les 2 types d'issue toujours bien représentés : 738 échecs de retour, 154 sorties avant touche valide).
- **Limite assumée** : non testable en rendu réel — retour nécessaire pour confirmer.
...
## v63 : Billard — mode paysage refait (vraie cause du bug identifiée), décor aligné sur le Ping-pong, mode Bot ajouté
L'utilisateur a diagnostiqué lui-même la cause : "je n'arrivais pas à faire viser la queue là où je voulais (ça bloquait en haut à droite), c'était quand la table était en mode paysage — actuellement en portrait, tout fonctionne parfaitement."
- **Cause confirmée en relisant le code** : `bilStart()` appelait `screen.orientation.lock('landscape')` (API peu fiable — échoue silencieusement sur iOS Safari, nécessite le plein écran ailleurs). Un commentaire dans le code confirmait qu'une "astuce CSS (rotation du conteneur)" avait été tentée par le passé puis désactivée après un bug ("le message de rotation ne disparaissait jamais") — ce genre de rotation CSS d'un élément + `getBoundingClientRect()` mal recalculé est la cause classique de ce type de bug de visée qui "bloque" dans un coin.
- **Fix : rotation au niveau du dessin, pas du DOM.** Le téléphone reste dans son orientation naturelle. Si l'écran est portrait (`vh > vw`), `bilDraw()` pose une matrice `ctx.setTransform()` qui fait pivoter le rendu de 90° pour donner l'apparence d'une table en paysage — l'élément `<canvas>` lui-même reste un rectangle CSS normal, jamais transformé, donc `getBoundingClientRect()` reste fiable. `bilGetPos()` applique l'inverse exacte de cette rotation (`bilCanvasToGame()`) pour convertir un clic en coordonnées "espace table" — tout le reste du code de jeu (visée, physique) continue de fonctionner en coordonnées 0..900/0..450 inchangées.
  - **Maths validées avant intégration** : script Node autonome testant le round-trip `jeu → canvas → jeu` sur plusieurs tailles de canvas et les 2 cas (pivoté/non pivoté) — erreur 0.000000 dans tous les cas, et les 4 coins de la table pivotée retombent bien dans les 4 coins du canvas.
  - Si l'écran est déjà large (`vh <= vw` — tablette, desktop, ou téléphone physiquement tourné), pas de rotation : affichage normal.
  - Taille du canvas recalculée dynamiquement (`bilResize()`, sur `visualViewport.resize`/`orientationchange`/`resize`) au lieu d'une résolution fixe 900×450 à l'ancienne, avec prise en compte du `devicePixelRatio` (bonus qualité, l'ancien rendu était flou sur écrans haute densité).
- **Décor aligné sur le Ping-pong** : même dégradé de fond, écran de fin remplacé par le même overlay (voile sombre, pluie d'étoiles, médaille or/argent, 3 boutons Accueil/Rejouer/⚙️).
- **Mode "Contre le bot" ajouté** (3e mode, en plus de Solo et Pass & Play). IA "ghost ball" : pour chaque bille jouable et chaque pochette, calcule le point de contact nécessaire, écarte les coupes trop fines (>~75°), choisit le meilleur candidat (distance courte + angle simple), ajoute une petite imprécision (~±2.5°) et une puissance variable, puis tire après un court délai (effet "réflexion"). Gère aussi le placement de la bille en main après une faute. Pas de vérification d'obstruction par les autres billes (simplification assumée — un tir bloqué se solde juste par un raté naturel dans le moteur physique).
  - **Testé séparément** : script Node de géométrie pure (sans moteur physique) — tir plein fouet détecté à 0° de coupe, 200 configurations aléatoires toutes cohérentes (angle ≤ 75°, direction toujours finie).
- **Limite assumée** : comme pour le Ping-pong, la sensation réelle du mode paysage (bonne fluidité de visée dans tous les coins de l'écran) n'est pas testable depuis ce sandbox — retour utilisateur nécessaire, en particulier sur le point précis qui avait posé problème ("ça bloquait en haut à droite").

## v64 : Billard — 3 correctifs après premier retour sur le mode Bot
- **Le bot bloquait parfois la partie** : cause identifiée — `bilAllStopped()` compare les vitesses avec `Math.abs(v)<0.02`, qui est TOUJOURS `false` si `v` est `NaN` (une comparaison avec NaN est toujours fausse). Si le bot calculait une direction de tir invalide (division dégénérée dans `bilBotPickShot`), `bilFireShot(NaN, NaN)` lançait la simulation avec une vitesse invalide, et `bilAllStopped()` ne redevenait alors jamais vrai → boucle d'animation infinie → partie plantée. Confirmé par un test dédié (`bil_watchdog_test.js`) : `bilAllStopped` avec une vitesse `NaN` retourne bien `false` indéfiniment.
  - **3 filets de sécurité ajoutés**, en profondeur :
    1. `bilFireShot()` rejette toute vitesse non finie (`NaN`/`Infini`) et retombe sur un tir neutre.
    2. `bilBotTakeShot()` vérifie que la direction calculée est finie et non nulle avant de tirer.
    3. **Garde-fou générique dans `bilAnimate()`** : si la simulation tourne depuis plus de 8 secondes sans se stabiliser (quelle qu'en soit la cause, même une future non prévue), on force l'arrêt de toutes les billes plutôt que de laisser la boucle tourner indéfiniment — protection large, pas seulement pour le cas du bot.
  - **Bonus robustesse** : `bilGameId` (jeton de session, incrémenté à chaque `bilStart()`/`bilBackToSetup()`) — si la partie est quittée ou relancée pendant le délai de "réflexion" du bot (700-900ms), le `setTimeout` en attente devient obsolète et ne déclenche plus rien sur la nouvelle partie.
- **Jauge de puissance saccadée en glissant** : la barre de remplissage avait une transition CSS (`transition:height 0.08s linear`) pensée pour lisser un changement ponctuel, mais qui accumule un effet de retard/rebond pendant un glissement continu rapide. Supprimée — le remplissage suit maintenant le doigt en 1:1 exact, sans délai.
- **Pochettes du milieu trop faciles** : ajout d'un rayon distinct et plus étroit pour les 2 pochettes latérales (`BIL_POCKET_R_SIDE = 24` contre `32` pour les pochettes de coin), comme sur une vraie table où les pochettes du milieu sont plus exigeantes en angle.
- **Testé** : script Node dédié confirmant le mécanisme du bug (NaN → `bilAllStopped` toujours faux) et l'efficacité des 2 filets (vitesse invalide neutralisée, garde-fou 8s qui se déclenche bien).
- **Limite assumée** : la cause exacte de la direction NaN du bot n'a pas pu être reproduite/isolée précisément (nécessiterait un vrai navigateur) — les filets de sécurité empêchent le blocage quelle que soit la cause, mais si le bot "rate" un peu plus que prévu dans de rares cas, ce sera visible en jeu réel plutôt qu'en simulation. Retour utilisateur nécessaire pour confirmer que le blocage ne se reproduit plus.

## v65 : Billard — le bot bloquait TOUJOURS (vraie cause probable trouvée) + animation de frappe de la queue
L'utilisateur confirme que le blocage persiste malgré les garde-fous de v64. Ré-analyse : les garde-fous de v64 protégeaient uniquement APRÈS `bilFireShot()` (vitesse NaN, simulation qui ne se stabilise jamais) — mais si l'exception se produit AVANT, directement dans `bilBotPickShot()` (appelée depuis un `setTimeout`), rien n'attrapait cette erreur : un `setTimeout` qui lève une exception l'écrit juste dans la console sans jamais planter la page, mais **`bilFireShot()` n'est alors jamais appelé du tout** — le tour du bot reste bloqué en `bilState='aim'` pour toujours, et aucun des garde-fous de v64 n'a la moindre chance de s'exécuter puisqu'ils vivent tous plus loin dans la chaîne. C'est très probablement ce que l'utilisateur observe avec "le bot ne joue parfois" : littéralement, le bot ne joue pas.
- **Fix : `bilBotTakeShot()` et `bilBotPlaceCue()` protégées par `try/catch`** — toute exception pendant le choix du coup ou du placement retombe désormais sur une action de repli sûre (tir neutre vers le haut, ou placement au centre de la table) plutôt que de laisser le tour du bot ne jamais se déclencher.
- **Garde-fou supplémentaire indépendant de la boucle d'animation** : un second `setTimeout` (6s), déclenché en parallèle de `bilFireShot()`, force l'arrêt même si une exception silencieuse dans `bilAnimate()` empêchait la boucle `requestAnimationFrame` de se relancer elle-même (auquel cas le garde-fou "temps écoulé" *à l'intérieur* de la boucle ne se déclencherait jamais, puisqu'il faut que la boucle continue de tourner pour le vérifier). `bilAnimate()` elle-même est maintenant aussi protégée par `try/catch`.
- **Animation de frappe de la queue ajoutée** (demande explicite) : nouvel état bref `'strike'` entre la visée et le tir réel — la queue (dessinée avec un dégradé bois clair→foncé, remplace l'ancienne ligne ambre plate) part de sa position reculée (proportionnelle à la puissance réglée) et fonce jusqu'à toucher la bille en ~130ms (`bilStartStrike`/`bilAnimateStrike`), et c'est SEULEMENT à la fin de cette animation que la vitesse réelle est appliquée à la bille. Fonctionne à l'identique pour le joueur (bouton TIRER) et pour le bot.
- **Testé** : re-validation syntaxique complète des 4 blocs `<script>`.
- **Limite assumée** : toujours pas de vrai navigateur pour reproduire le cas précis qui faisait planter `bilBotPickShot()` — les 2 niveaux de `try/catch` + le double garde-fou (dans la boucle ET indépendant) couvrent toutes les classes de blocage identifiables par lecture de code, mais un retour utilisateur reste nécessaire pour confirmer que ça ne se reproduit plus.
