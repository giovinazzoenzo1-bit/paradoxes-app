...
- **Monnaie in-game globale** (`appCoins`, une seule cagnotte partagée par toute l'app, barre fixe toujours visible en haut de l'écran). Formule de gain pour les jeux avec bot, calibrée sur **120 pièces/heure** : `pièces = 2 × durée_partie(min) ÷ P(victoire)`. Pour les jeux sans bot (score/temps) : paliers fixes atteints une fois par partie. Un vrai Expert imbattable sort de la formule (P=0) : traité comme trophée/diamant hors farm.
  ...
  - **Boutique de raquettes Ping-pong** (inspirée du jeu concurrent "Ping Pong Go!" — observation depuis des captures d'écran fournies par l'utilisateur, aucun code récupéré/copié) : 8 skins de couleur achetables avec les pièces (0 à 75), cosmétique uniquement, liste éditable dans `coins-config.js` (`pingpongPaddles`). Bouton rond en bas à droite de l'écran de config. **Fond néon (rose/cyan/violet) étendu à toute la section Ping-pong** (config, table de jeu, scoreboard, écran de victoire — pas seulement l'écran de config). Vignettes de la boutique refaites en tête+manche pour plus de réalisme.
  - **Boutique de fonds Flappy Bird** (même principe) : 6 thèmes de ciel + couleur des tuyaux (Classique gratuit → Néon 40 pièces), certains avec étoiles, liste éditable dans `coins-config.js` (`flappyBirdThemes`). Bouton "🎨 Fonds" à côté de "Nouvelle partie".
  - **Sprites Flappy Bird réorganisés** dans `sprites/flappybird/` (dossier dédié, au lieu d'un fichier plat) : `bird_toucan.png`, `pipe_body.png` (52×42, tuilé verticalement, dernière tuile recadrée jamais étirée), `pipe_cap.png` (60×24, dépasse de 4px de chaque côté). **Tuyaux texturés actifs uniquement sur le thème "classique"** — les 5 autres thèmes de la boutique n'ont pas encore d'assets dédiés, ils gardent un remplissage couleur uni en attendant.
  - **Système "retirer les pubs" corrigé (pas supprimé)** : l'utilisateur voulait juste masquer la bannière au lancement pour un meilleur confort de test/dev — le mock d'achat 2,99€ et le toggle restent fonctionnels dans Options, seule la valeur PAR DÉFAUT change (`adsRemoved` = true dès le premier lancement, sauf si explicitement désactivé via le bouton). Un essai précédent avait supprimé tout le système par erreur, restauré ensuite.
  - **Mode développeur** ajouté tout en bas d'Options : bouton "+10 pièces" qui contourne volontairement le plafond anti-abus de 40/h (outil de test, pas une action de jeu normale).
