...
**Morpion** — cahier des charges complet lu (Drive, docs "Enzo"=logique et "Flavio"=UI/visuel). L'ancienne version était un round unique minimal ; réécrit pour suivre le §1/§2/§4 du cahier des charges : match BO3 (premier à 2 manches gagnées), alternance du 1er joueur à chaque manche, Undo (1 gratuite/manche puis pub pour +2), ligne de victoire animée (SVG, se "déroule"), anti-spam sur case occupée (tremblement+vibration), streaks locaux (flamme dès 3 manches d'affilée), économie de pièces (+10/manche, +50/match, +5 consolation perdant), doublement des pièces via pub en fin de match. Choix de design documentés dans le commit : cagnotte de pièces partagée par l'appareil (party game 2P local, pas de profil séparé par joueur), Undo scope par manche (pas cross-manches). Pas encore fait : §3 sound design dédié (réutilise les sons génériques existants pour l'instant, pas de synthèse audio custom par événement), reskin visuel Flavio (assets PNG 512×512 pas encore fournis), roadmap V2 du cahier des charges (bot IA, anti-match-nul 3 pions) explicitement marquée "MAJ suivante" donc pas traitée.
...
- ✅ Mode Rush 60s (marqué "future MAJ" dans le cahier des charges mais demandé maintenant par l'utilisateur) : chrono 60s, +1s par fusion réussie (plafonné à 60), classement séparé du mode Classique (panneau à onglets, perso signé anti-triche + mondial aperçu).
