...
// ⚠️ 120 -> 84 : baisse de 30 % demandée le 19/09, appliquée à TOUS les
// niveaux d'un coup puisque la courbe est `base x 2^(niveau-1)`.
export const PACTE_BASE_COST = 84;
...
export function passiveRate({
  ...
  const commun = base
    ...
    * ascensionPassiveMultiplier(ascensionCount || 0);
...
export const TAP_UPGRADES = [
  { id: 'tap1', name: 'Poigne Ancienne', emoji: '✊', bonus: 0.194, cost: 847, growth: 1.45 },
  { id: 'tap2', name: 'Gantelet d\u2019Obsidienne', emoji: '🪄', bonus: 0.317, cost: 1623, growth: 1.45 },
  { id: 'tap3', name: 'Sceau de Puissance', emoji: '🔱', bonus: 0.165, cost: 989, growth: 1.45 },
  { id: 'tap4', name: 'Poing de Granit', emoji: '🗿', bonus: 0.269, cost: 1896, growth: 1.45 },
  { id: 'tap5', name: 'Éclat Primordial', emoji: '💠', bonus: 0.178, cost: 1479, growth: 1.45 },
  { id: 'tap6', name: 'Coeur de Supernova', emoji: '🌟', bonus: 0.291, cost: 2835, growth: 1.45 },
  { id: 'tap7', name: 'Griffe du Vide', emoji: '🕳️', bonus: 0.202, cost: 2319, growth: 1.45 },
  { id: 'tap8', name: 'Serment Éternel', emoji: '♾️', bonus: 0.327, cost: 4446, growth: 1.45 },
  { id: 'tap9', name: 'Fracture du Réel', emoji: '⚡', bonus: 0.249, cost: 3959, growth: 1.45 },
  { id: 'tap10', name: 'Volonté du Paradoxe', emoji: '🌌', bonus: 0.404, cost: 7590, growth: 1.45 },
];
...
// ⚠️ `creatureId` inscrit EXPLICITEMENT et non déduit du nom : la
...
// ⚠️⚠️ BONUS RÉDUIT SUR LE REVENU PASSIF — racine carrée du bonus.
//
// Le bonus d'Ascension multipliait AUSSI les générateurs. Deux effets se
// composaient donc à chaque Ascension : le joueur débloque un palier
// supérieur (x5 de rendement) ET tout est multiplié par le bonus. Mesuré,
// la production montait x321 par groupe quand les seuils ne pouvaient
// suivre qu'à x82 — c'est ce qui vidait les groupes de leur durée.
//
// Le supprimer entièrement du passif désavantageait trop les
// générateurs. La racine carrée est le compromis retenu : le TAP garde
// le bonus plein (x210 à A5, la sensation reste entière), le passif ne
// monte plus que x1,9 par Ascension au lieu de x3,5.
export function ascensionPassiveMultiplier(ascensionCount) {
  return Math.sqrt(ascensionSpeedMultiplier(ascensionCount));
}
...
// ⚠️⚠️ PRIX PROPORTIONNELS AU RENDEMENT, avec un FREIN de 15 % par palier.
//
// AVANT : chaque palier rapportait x5 mais coûtait x9, donc il était
// 42 % MOINS rentable que le précédent. Mesuré sur les 15 générateurs,
// sans une seule exception — acheter un Golem était mathématiquement
// moins bon qu'acheter des Esprits Frappeurs. C'est pour ça que 10
// générateurs sur 15 n'étaient JAMAIS achetés, quel que soit l'argent
// disponible. Le problème n'a jamais été le pouvoir d'achat.
//
// ⚠️ MAIS UNE RENTABILITÉ ÉGALE NE MARCHE PAS NON PLUS. Sans écart, le
// joueur enchaîne les 15 paliers en un seul groupe : chaque palier
// finance le suivant et la production explose. La baisse de rentabilité
// n'était donc pas un bug, c'était un FREIN — il était simplement dix
// fois trop fort.
//
// Réglage retenu : chaque palier est 15 % moins rentable que le
// précédent (contre 42 % avant). Assez pour freiner l'escalade, assez
// peu pour que tous les paliers restent désirables.
//
// ⚠️ Prix et rendement doivent rester PROPORTIONNELS à ce frein près.
// Changer l'un sans l'autre remet un palier hors course.
// ⚠️⚠️ RENDEMENTS À x2,2 PAR PALIER — choix de l'auteur du 19/09.
//
// Ils étaient à x5. Or le joueur découvre DEUX nouveaux générateurs par
// Ascension, soit x25 de rendement par groupe, quand le seuil
// d'Ascension ne monte que de x5 à x8. Les paliers s'éloignaient donc
// 3 à 5 fois plus vite que le pouvoir d'achat.
//
// Mesuré : à partir d'A2, AUCUN exemplaire du générateur du groupe
// n'était abordable dans 20 % du seuil. Le défi « Possède 45 Titans de
// Foudre » coûtait 16 millions de fois le seuil — l'auteur a mis 20
// minutes à en acheter UN.
//
// À x2,2, deux paliers font x4,8 par groupe : la boutique suit enfin le
// pouvoir d'achat, et le plan « 2 générateurs découverts par Ascension »
// devient réalisable.
//
...
export const AUTOCLICKERS = [
  { id: 'esprit', name: 'Esprit Frappeur', emoji: '👻', baseCost: 7569, baseIncome: 6.98, tier: 1 },
  { id: 'main', name: 'Main Spectrale', emoji: '🖐️', baseCost: 13734, baseIncome: 10.8, tier: 1 },
  { id: 'automate', name: 'Automate Runique', emoji: '⚙️', baseCost: 13023, baseIncome: 8.66, tier: 1 },
  { id: 'colonie', name: 'Colonie de Familiers', emoji: '🦊', baseCost: 18152, baseIncome: 10.3, tier: 1 },
  { id: 'titan', name: 'Titan Mécanique', emoji: '🗿', baseCost: 19475, baseIncome: 9.37, tier: 1 },
  { id: 'golem', name: 'Golem de Cristal', emoji: '💎', baseCost: 27797, baseIncome: 11.4, tier: 2 },
  { id: 'dragonnet', name: 'Dragon Miniature', emoji: '🐉', baseCost: 30540, baseIncome: 10.6, tier: 2 },
  { id: 'phenix', name: 'Phénix Renaissant', emoji: '🔥', baseCost: 45482, baseIncome: 13.4, tier: 2 },
  { id: 'leviathan', name: 'Léviathan des Abysses', emoji: '🐋', baseCost: 52139, baseIncome: 13.1, tier: 2 },
  { id: 'gardien', name: 'Gardien Céleste', emoji: '👼', baseCost: 146966, baseIncome: 31.3, tier: 2 },
