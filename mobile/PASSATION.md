...
- **2b-2 FAIT** : photo des 3 meilleures créatures prise au DÉBUT de chaque œuf (`gardienPhoto`, sauvegardée avec l'œuf ; prise au chargement pour les œufs d'avant) ; calibrage au LANCEMENT du combat sur cette photo (≈ 0,2 s, même Gardien à chaque essai), œuf 2 = ancien Gardien ; « ⚔️ Gardien X · 🛡️ Ton deck Y » sous le bouton (œuf principal et incubateur) ; « 🛡️ Puissance » dans l'en-tête de l'Aventure. Simulation deux fois plus rapide (combattants préparés une fois), résultats identiques au chiffre près.
...
- **Bug dangereux signalé par l'auteur, corrigé (24/09)** : placer une créature dans le deck la rendait prête tout de suite (pouvoirs gratuits en changeant de créature). `rechargesApresChangementDeck` : toute ARRIVÉE démarre la recharge complète (jamais raccourcie ; retirer puis remettre relance à fond ; changer de place n'est pas une arrivée), appliquée sur l'ÉTAT du deck (écran principal, Exploration, remplissage auto, tout futur chemin) ; le deck chargé sert de référence. `auditRechargeDeck` (règle + câblage, 2 sabotages).
- **Reste** : le test de l'auteur.

## 3 quater. MESURE EN COURS — la difficulté de l'Aventure (24/09)
...
## 3 quinquies. CHANTIER EN COURS — les SORTS des créatures (24/09)
Décisions de l'auteur : chaque créature reçoit UN sort payé en MANA (grisé si insuffisant, départ à 2 de mana) ; les ennemis de l'Aventure ont aussi le leur (pas le Gardien pour l'instant). Chiffres : bouclier 45 % des PV perdus (plus blessé), soin 30 % des PV perdus (plus blessé), Venin (frappe à 70 %, lanceur venimeux 2 tours : qui le frappe perd 30 % d'attaque et 5 % PV/tour pendant 2 tours ; l'ennemi l'évite s'il a une autre cible), zone 40 % par ennemi, boost +35 % sur les 3 prochaines attaques de l'allié le plus fort, vitesse −35 % de taps (ne coûte pas le tour), marque +35 % sur 2 coups, exécution ×2 sous 30 % sinon ×0,6, pacte ×2 pour 15 % de PV, provocation 2 tours −20 %, Fureur +50 % après 30 tours. Répartition (26 créatures) : Zone pyrosile/ventis/voltix/solarion ; Exécution ombrillon/abyssorax/solstral ; Pacte terracroc/malefix/arcanis ; Bouclier caraploof/aegisolar/runicor ; Venin fournax/cumulox/tartaroth ; Provocation bouldog/brontobloc ; Soin luxorbe/aquamira ; Boost braiserose/voltarel ; Vitesse zephyrion/racinea ; Marque glyphon/nocturis. Futurs : Étourdissement, Siphon, Résurrection.
- **Étape 1 FAITE (moteur, non branché)** : `combatLogic` — SORTS, EFFETS, MANA_DEPART, lancerSort, frapper, cibleDeRiposte / cibleDuJoueur (TOUJOURS une cible), finDeTour, multiplicateurFureur, tapsAvecEtats, iconesEtats. `auditSortsMoteur` : chaque chiffre + 2 000 combats aléatoires (cible, PV sains, fin) ; 2 sabotages.
- **Étape 2 FAITE** : `SORT_DE_CREATURE` (clickerLogic, ids) + `competencesAvecSort` (combatLogic) : le sort remplace la compétence normale la plus faible, meilleure attaque et spéciale gardées ; l'ancienne zone GRATUITE des Soutiens (2e compétence : 100 % sur TOUS les ennemis, 300 % contre 3) disparaît de cette liste. `creature.skills` INCHANGÉ tant que l'écran n'est pas passé à `competencesAvecSort` (étape 3) : aucun bouton sans dégâts entre deux mises à jour. `auditSortsDonnees` + sabotage.
- **Étape 3a FAITE (moteur branché, pas encore de boutons)** : CombatScreen — mana de départ MANA_DEPART pour tous (et plus de +1 à la première créature, dans les simulations non plus : c'était un écart), `etats` sur chaque combattant, riposte vers `cibleDeRiposte` via `frapper` × Fureur, `finDeTour` sur les deux équipes, K.O. possible hors créature active (lu dans `newFighters`), taps via `tapsAvecEtats`, ligne d'états (`iconesEtats`) en absolu à DROITE de la barre de PV de nos créatures, à GAUCHE des adversaires. Simulations alignées (mana, Fureur). Gardien recalé tout seul (contrôle vert). Empreinte mise à jour après revérification.
- **Étape 3b** : écran — coût en mana, sorts grisés ; PETITE LIGNE D'ÉTATS à côté de chaque barre de PV (à DROITE pour nos créatures, à GAUCHE pour les adversaires) avec l'icône de chaque bonus/malus (`iconesEtats`, emojis en attendant les logos de l'auteur).
- **Étape 4** : logique des sorts ennemis (Aventure seulement). **Étape 5** : simulations (le Gardien se recale), puis calibrage de l'Aventure.
